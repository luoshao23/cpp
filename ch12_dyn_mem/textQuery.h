...
public:
    TextResult(std::string s, std::shared_ptr<std::set<line_no >> p,
...
public:
    ...
    TextResult query(std::string &s) const;
...
TextResult TextQuery::query(std::string &s) const
{
    static std::shared_ptr<std::set<line_no >> nodata(new std::set<line_no>);
    auto loc = word_map.find(s);
    if (loc == word_map.end())
        return TextResult(s, nodata, content);
    else
        return TextResult(s, loc->second, content);
...
std::ostream &print(std::ostream &os, const TextResult& qr){
